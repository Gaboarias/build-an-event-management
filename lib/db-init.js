...
async function init() {
  ...
  // Add frozen column to event_zones if missing
  await sql`
    ALTER TABLE event_zones
      ADD COLUMN IF NOT EXISTS frozen BOOLEAN NOT NULL DEFAULT false;
  `;

  // Add frozen_zones to event_config if missing
  await sql`
    ALTER TABLE event_config
      ADD COLUMN IF NOT EXISTS frozen_zones TEXT NOT NULL DEFAULT '[]';
  `;

  // Academia students — monthly fee tracking
  await sql`
